...
| `uplink` (`name`, `port_idx`, `mac`, `ip`, `speed`, `up`, `type: wire`, `rx/tx`, `uplink_mac`, `uplink_remote_port`) | which port faces the controller | port 49 (LLDP to the aggregation switch) | sent; **controller stores `{}`** — it derives the uplink from `lldp_table` itself (topology is correct). Low value; leave |
