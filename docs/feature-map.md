...
|---|---|---|---|
| `mac`, `serial`, `model`, `model_display`, `version`, `ip`, `hostname`, `inform_url` | identity | `show version`, `show hostname`; model string claimed (UDC48X6) | done |
| `uptime`, `time` | clocks | `show version` uptime | done |
| `cfgversion`, `x_authkey`, `default`, `_default_key`, `state`, `fw_caps` | protocol state | session | done |
| `sys_stats` (`cpu`, `mem_total`, `mem_used`, `mem_buffer`) | health | `show processes top once` | done |
| `general_temperature`, `has_temperature` | temperature | `show system environment temperature` (hottest sensor) | done |
| `overheating` | bool | same command, `systemStatus != temperatureOk` | done |
| `fan_level`, `has_fan`, `fan_table` | cooling | `show system environment cooling` | done (controller shows fan_level; `fan_table` rows use the real keys: index, label, present, status, speed, target_speed) |
| `psu_table`, `total_max_power`, `power_consumption` | power supplies | `show system environment power` | done — rows must use the real keys (`psu_idx`, `label`, `present`, `online`, `power`, `power_capacity`, `temp`); with invented keys the UI showed both supplies as "Not Installed" (fixed 2026-09-19) |
| `total_max_power`, `power_source`, `psu_*` | PSU | `show system environment power` | done (`total_max_power` stored; `power_consumption` ignored) |
| `port_table` | per port, see §2 | `show interfaces` + status + STP + LLDP | done |
...
| `uplink` (`name: eth0`, `port_idx`, `mac`, `ip`, `netmask`, `speed`, `up`, `type: wire`, counters), `if_table` | which port faces the controller | port 49 (LLDP to the aggregation switch) | the controller ignores the device's report and derives its own (`uplink_source: lldp_uplink`/`lldp_downlink`). State 2026-09-20: the parent's `downlink_table` lists us on its port 49, the device panel shows "Parent Device: the aggregation switch Port 49", clients behind us get `sw_port`, and the topology map hangs us under the parent — but our own `uplink` stays counters-only, so the list's Uplink/Parent columns and the panel's "Connected To" are blank. Needed for the rest to work: management address **in-band** (§2c of adding-a-switch.md) and no LLDP on the OOB port. Still-open hypothesis: the controller derives a child's port from the parent's LLDP `port_id` and cannot parse ours ("Ethernet49/1" → trailing "1"); or the USW Leaf model gets leaf/spine handling (test = re-adopt as USWF066, Clint's call) |
...
| `dot1x_portctrl_enabled` | 802.1X | n/a unless configured; report false | todo |
| `dhcp_server_table`, `ip_table` | L3 features | Arista is pure L2 here | n/a |
| `ssh_session_table` | active SSH sessions | `show users` | todo (cosmetic) |
| `led_override`, `locating` | LED/locate | no LED control via eAPI on 7160 | report false |
| `speedtest-status`, `isolated`, `selfrun_beacon`, `bootrom_version` | misc | static | done |

## 2. Device → controller: `port_table` per port

| Field | Arista source | Status |
|---|---|---|
| `port_idx`, `ifname`, `name`, `media`, `is_uplink`, `poe_caps`, `port_poe` | profile + collector | done |
| `up`, `enable`, `speed`, `full_duplex`, `mtu`, `stp_state` | `show interfaces`, `show spanning-tree` | done |
| `rx_bytes/packets/errors/dropped`, `tx_*` | `interfaceCounters` | done |
| `rx_multicast`, `rx_broadcast`, `tx_multicast`, `tx_broadcast` | `interfaceCounters` | done |
| `sfp_found` | line protocol `notPresent` | done |
| `sfp_vendor`, `sfp_part`, `sfp_serial`, `sfp_compliance` | `show inventory` (`xcvrSlots`) + `show interfaces transceiver` (`mediaType`, `vendorSn`) | done (controller shows vendor/part) |
| `sfp_temperature`, `sfp_voltage`, `sfp_current`, `sfp_rxpower`, `sfp_txpower` | `show interfaces transceiver` (`temperature`, `voltage`, `txBias`, `rxPower`, `txPower`) | done; DAC cables (port 53) have no DOM, so those stay absent |
| `autoneg` | `show interfaces status` `autoNegotiateActive` | done. `speed_caps` todo |
| `flowctrl_rx`, `flowctrl_tx` | `show interfaces flow-control` | done |
| `jumbo` | mtu > 1518 | done |
...
| `mac_table` per port | `show mac address-table` | done |
| `fec_mode` (`rs-fec`/`fc-fec`/`disabled`) | `show interfaces error-correction` (`errCorrEncodingStatus`: reedSolomon / fireCode / disabled) | **done, reported** (controller stores it: 49/51/53 rs-fec, breakout lanes fc-fec). Apply side needs the UI capture |
| `poe_*` | n/a (no PoE) | n/a |
...
| Key family | Meaning | EOS translation | Status |
|---|---|---|---|
| `switch.port.N.status=enabled/disabled` **obs** | admin state | `[no] shutdown` | done |
| `switch.port.N.name` **obs** | port name | `description` (controller default names `SFP28 N`/`QSFP28 N` and profile `Port N` → `no description`) | done, verified live |
| `switch.port.N.ld_mode=disabled` **obs** (loop detection?) | | ignore for now | verify |
| `switch.port.N.opmode=aggregate` + `switch.port.N.lag=<id>` **obs** | link aggregation | `channel-group <id> mode active` on every lane; the Port-Channel carries the VLAN config | **done, verified live** (ports 2+3) |
| `switch.port.N.opmode=mirror` + `switch.port.N.mirror_port=<src>` **obs** | port mirroring (N is the destination) | `monitor session <N> source Ethernet<src> both` / `destination Ethernet<N>` | **done, verified live** |
| `switch.vlan.<slot>.port.N.mode=untagged\|tagged\|exclude` **obs** (slot → `switch.vlan.<slot>.id`; no lines = "Allow All": every VLAN tagged, VLAN 1 untagged) | per-port VLAN membership | trunk + `switchport trunk native vlan` + `switchport trunk allowed vlan <list>\|all`; "Block All" (native only) = access port | **done, verified live** (port 2: native 1, tagged 2+10 → `allowed vlan 1-2,10`) |
| `switch.vlan.N.id/mode/status` **obs** | site VLAN list | `vlan <ids>` created before ports reference them; extra VLANs on the switch are left alone | **done, verified live** (69, 4000 created) |
| `switch.managementvlan` **obs** | mgmt VLAN | Ma1 is out-of-band; report only | report |
| `switch.mtu`, `switch.jumboframes` **obs** | jumbo | 7160: no per-port L2 MTU (`l2 mtu` unsupported); always forwards jumbo; logged when UniFi asks for off | report only |
| `switch.port.N.autoneg=disabled` + `speed=10000` + `duplex=enabled` **obs** (absent = auto) | link speed | `speed forced 10gfull` (keyword table in `eos/vlan.go`); auto → `speed auto` on copper only — optical ports run forced by hand and are never set to auto by the bridge | **done, verified live** |
| `switch.port.N.fec=cl-91\|cl-74` **obs** (absent = leave alone) | FEC | `error-correction encoding reed-solomon\|fire-code`; `no error-correction encoding` on explicit disable | **done, verified live** (port 54) |
| `switch.port.N.flowctrl.*` **verify** | flow control | `flowcontrol receive/send on\|off` | todo |
| `switch.port.N.isolation` **verify** | port isolation | `switchport port-security`? / protected ports — EOS lacks a direct equivalent; ACL-based | verify |
| `switch.stp.status/version/priority`, `switch.port.N.stp.port_mode`, `.stp.bpdu_guard` **obs** | STP | `spanning-tree mode rstp\|none`, `spanning-tree priority N`; port_mode=disabled → `spanning-tree portfast` + `bpdufilter enable`; bpdu_guard → `spanning-tree bpduguard enable` | **done, verified live** |
| `switch.port.N.stormctrl.status/type/bcast/mcast/ucast` **obs** | storm control (percent) | `storm-control broadcast\|multicast\|unknown-unicast level X` | **done, verified live** |
| `switch.port.N.lldpmed.opmode` **obs** | LLDP-MED | no per-port MED toggle on EOS 4.26; logged when disabled | report only |
| `switch.port.N.dot1x.*`, `switch.dot1x.*` **verify** | 802.1X | out of scope unless asked | n/a |
| `switch.port.N.poe.*` | PoE | n/a | n/a |
| `switch.vlan.<slot>.igmp_*` **obs** | IGMP snooping per VLAN | `[no] ip igmp snooping vlan N` (`-control-igmp`) | **done, verified live** |
| `switch.dhcp_snoop.status` **obs** | DHCP snooping | `ip dhcp snooping` + `ip dhcp snooping vlan <all switch VLANs>` | done (apply written; capture of enabled state pending) |

| `ntpclient.N.server` **obs** | NTP | `[no] ntp server X` (`-control-ntp`) | **done, verified live** |
| `syslog.status/ip/port` **obs** | remote syslog | `[no] logging host X [port]` (`-control-syslog`) | done (no host set in UniFi yet) |
| `snmp.*` **verify** | SNMP | `snmp-server community` | todo |
| `sshd.auth.key.N.*` **obs** | site SSH keys | `username <bridge user> ssh-key [secondary] …` (EOS: 2 keys per user; `-control-ssh-keys`) | **done, verified live** |
| `users.N.name/password` **obs** (MD5-crypt), `sshd.auth.key.N.*` | device login for the UniFi terminal | **Not served.** The UI's Manage → Debug terminal is a WebRTC session the controller asks the device to build (`build-ssh-session` inform cmd), not SSH to the device IP; it is hidden by not claiming `fw_caps` UTERM (4). An SSH gateway that honours these credentials over plain SSH is parked on branch `ssh-gateway` (`docs/ssh-gateway-status.md` there). Keys are still installed on the switch user (`-control-ssh-keys`) | parked |
| `unifi.*`, `netconf.*`, `dhcpc.*`, `resolv.*`, `route.*`, `bridge.*` **obs** | UniFi OS internals | ignore | n/a |
| `system.timezone`, `locale.timezone` **obs** | clock | `clock timezone` | todo (safe) |
